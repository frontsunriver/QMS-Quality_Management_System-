...
function onConfirmOrder(id) {
	bootbox.confirm("Do you really confirm this purchase order?", function(result){
		if (result) {
			location.href = base_url + 'manufacture/purchaseorder/confirm/' + id;
		}
	});
}
...
function onAddLine() {
	...
	row[1] = '';
	row[2] = '<input type="text" class="form-control form-control-sm col-lg-12" id="anytime' + pIndex + '" name="material[' + pIndex + '][scheduled_date]" value="" style="background-color: white;">';
	row[3] = '<input type="number" class="form-control form-control-sm format-unit-price" name="material[' + pIndex + '][quantity]" value="1" onchange="javascript:onCalcTotal();">';
	row[4] = '<input type="number" class="form-control form-control-sm format-unit-price" name="material[' + pIndex + '][unit_price]" value="10" onchange="javascript:onCalcTotal();">';
	row[5] = '<input type="number" class="form-control form-control-sm format-tax" name="material[' + pIndex + '][tax]" value="15" onchange="javascript:onCalcTotal();">';
	row[6] = '';
	row[7] = '<ul class="icons-list text-center"><li class="text-danger-400" style="margin-left: 0px;"><i class="icon-trash" onclick="javascript:onDeleteMaterial(this);"></i></li></ul>';
...
function onChangeMaterial(obj) {
	var value = $(obj).val();
	$.post(base_url + 'manufacture/purchaseorder/get/' + value, {}, function(resp) {
		var tr = $(obj).parent().parent();
		$($(tr).children()[1]).text(resp.supplier_name);
...
function onCalcTotal() {
	...
	for (var i = 0; i < trs.length; i ++) {
		if (!$($(trs[i]).children()[0]).hasClass('dataTables_empty')) {
			...
			$($(trs[i]).children()[6]).text('$ ' + unit_price * quantity);
			...
			var total = parseFloat($($(trs[i]).children()[6]).text().split(' ')[1]);

...
$(function(){
	var table = $('.datatable-customer').DataTable({
		...
		"aoColumns" : [{
			...
			"sTitle" : "Product Category Name", "mData": "name", "sWidth": "25%"
		}, {
			"sTitle" : "Description", "mData": "description", "sWidth": "50%"
		}, {
	...
	$("#add_form").validate({
		...
		submitHandler: function (form) {
			...
			$.post(base_url + 'manufacture/category/add', params, function(resp){
				if (resp.success) {
					new PNotify({
						title: 'Success',
...
function onAdd() {
	$("#modal_save .modal-body").load(base_url + 'manufacture/category/add', {}, function(){
		$("#modal_save .modal-title").text("Add Product Category");
...
function onEdit(id) {
	$("#modal_save .modal-body").load(base_url + 'manufacture/category/add', {id: id}, function(){
		$("#modal_save .modal-title").text("Edit Product Category");
...
function onDelete(id) {
	 bootbox.confirm("Do you really delete this category?", function(result){
	 	if (result) {
	 		$.post(base_url + 'manufacture/category/delete/' + id, {}, function(resp, status){
	 			if (resp.success) {
					new PNotify({
						title: 'Success',

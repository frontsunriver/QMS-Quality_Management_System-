$(function() {
    ...
    var table = $('.datatable-assessment-controls').DataTable({
        "bServerSide": true,
        "bProcessing": true,
        "aoColumns": [
                    {
                    ...
                    { "sTitle" : "RATING", "mData": "rating", "sWidth": 120 },
                    { "sTitle" : "ACTION", "mData": "action", "sWidth": 120 },
        ...
        "bAutoWidth": false,
        "sAjaxSource": 'assessment_controls_read',
        "sAjaxDataProp": "assessment_controls",
        scrollX: false,
        scrollCollapse: true,
        "order": [
            [1, "asc"]
    ...
    $('.datatable-assessment-controls tbody').on('click', 'a[title="Edit"]', function () {
        var data = table.row($(this).parents('tr')).data();
        $('#info_form')[0].reset();
        $('#info_form input[name="id"]').val(data.id);
        $('#info_form input[name="rating"]').val(data.rating);
        $('#info_form input[name="action"]').val(data.action);
        $('#info_form textarea[name="description"]').val(data.description);
        $('#modal_save').modal('show');
    });

    $('.datatable-assessment-controls tbody').on('click', 'a[title="Delete"]', function () {
        ...
        bootbox.confirm("Are you sure?", function(result) {
            if (result) {
                ...
                $.post('assessment_controls_delete', params, function(data, status){
    ...
    var validator1 = $("#info_form").validate({
        ...
        submitHandler: function (form) {
            ...
            $.post('assessment_controls_save', params, function(data, status){
                if (data.success) {
                    new PNotify({
                        title: 'Success',

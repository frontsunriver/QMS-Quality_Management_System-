$(function() {
    ...
    var table = $('.datatable-customer').DataTable({
        ...
        "bAutoWidth": false,
        "sAjaxSource": 'regulatory_requirement_read',
        "sAjaxDataProp": "regulatory",
        scrollX: true,
        scrollCollapse: true,
        "order": [
            [1, "asc"]
    ...
    $('.datatable-customer tbody').on('click', 'a[title="Delete"]', function () {
        ...
        bootbox.confirm("Are you sure?", function(result) {
            if (result) {
                ...
                $.post('delete_regulatory_requirement', params, function(data, status){
                    if (status == "success") {
                        new PNotify({
                            title: 'Success',
    ...
    var validator1 = $("#info_form").validate({
        ...
        submitHandler: function (form) {
            ...
            $.post('edit_regulatory_requirement', params, function(data, status){
                if (data != "") {
                    new PNotify({
                        title: 'Success',

	...
	var table = $('.datatable-shiporder').DataTable({
		...
		"aoColumns" : [{
			...
			mRender: function(data, type, row) {
				if (data == 0)
					return '<label class="label label-danger" style="margin-bottom: 0px; cursor: pointer;" onclick="javascirpt:onEdit(' + row.id + ');">Waiting Another Operation</label>';
				else if (data == 1)
					return '<label class="label label-success" style="margin-bottom: 0px;">Sale Done</label>';
			}
		}],
...
function onCreate() {
	location.href = base_url + 'warehouse/shiporder/create';
}

function onEdit(id) {
	location.href = base_url + 'warehouse/shiporder/create/' + id;	
}
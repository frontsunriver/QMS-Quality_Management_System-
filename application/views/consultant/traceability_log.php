...
<script type="text/javascript">
    ...
    function scan(){
        ...
        $('#url').val("<?php echo  $_SERVER["HTTP_HOST"]."".base_url(); ?>index.php/welcome/upload_barcode/machine/" + id);
        $('#modal_value').modal();
        start_scan(id);
    }
    $(function(){
    ...
    function get_image(){
        var machine_id = $('input[name="id"]').val();
        $.ajax({
            type: "POST",
    ...
    $(function() {
        ...
        var table = $('.datatable-customer').DataTable({
            ...
            "aoColumns": [
                {
                    "sTitle" : "NO", "mData": "id","sWidth": 30,
                    mRender: function (data, type, row, pos) {
                        var info = table.page.info();
                        return Number(info.page) * Number(info.length) + Number(pos.row) + 1;
                    }
                },
                /*{ "sTitle" : "Control Name", "mData": "control_id", "sWidth": 300 },*/
                { "sTitle" : "Product Name", "mData": "product_name", "sWidth": 300 },
				{ "sTitle" : "Product Quantity", "mData": "product_quantity", "sWidth": 300 },
                { "sTitle" : "Product Barcode Info", "mData": "product_barcode", "sWidth": 300 },
                {
                    "sTitle" : "Actions", "mData": "",
                    mRender: function (data, type, row) {
                        return  '<ul class="icons-list">' +
                                '<li><a href="show_control_barcode/'+row.id+'" type="button" title="Edit" class="btn btn-primary btn-sm" style="color: white;">Detail</a></li>' +
                            '</ul>';
                    }
                }
            ],
            ...
            "bAutoWidth": false,
            "sAjaxSource": 'traceability_log_read',
            "sAjaxDataProp": "traceability",
            scrollX: true,
            scrollCollapse: true,
            "order": [
                [1, "asc"]
        ...
        $('.datatable-customer tbody').on('click', 'a[title="Delete"]', function () {
            ...
            bootbox.confirm("Are you sure?", function(result) {
                if (result) {
                    ...
                    $.post('delete_machine', params, function(data, status){
                        if (status == "success") {
                            new PNotify({
                                title: 'Success',
        ...
        var validator1 = $("#info_form").validate({
            ...
            submitHandler: function (form) {
                ...
                $.post('edit_machine', params, function(data, status){
                    if (data != "") {
                        new PNotify({
                            title: 'Success',

...
class Customer extends MY_Controller {
	...
	public $prefix_num = 'CU/';
	public $num_length = 4;

	function index() {
		$this->mHeader['title'] = 'Customer Management';
		$this->mHeader['menu_id'] = 'customer';
		$this->render('manufacture/customer/list', $this->mLayout);
	}
	...
	function read() {
		...
		$data['customer'] = $this->Ims_customer_model->find($filter, $orders);
	...
	function add() {
		...
		if (!$param) {
			...
			$this->load->view('manufacture/customer/add', $data);
		} else {
			if ($id == -1) {
				$reference = $this->Ims_customer_model->select_max('reference');
